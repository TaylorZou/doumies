...
import static com.doumies.mcs.pojo.constant.AppConstants.COUPON_LOCK;
...
@Service
@Slf4j
public class CouponRecordServiceImpl extends ServiceImpl<McsCouponRecordMapper, McsCouponRecord> implements ICouponRecordService {
    ...
    public void add(String couponId) {
        ...
        try {
            McsCoupon coupon = couponService.getById(couponId);
            this.couponCheck(coupon, userId);
            // 封装优惠券领取记录对象
            McsCouponRecord couponRecord = new McsCouponRecord();
...
//            couponRecord.setUseState(CouponStateEnum.NEW.name());
     ...
     * @param userId
     */
    private void couponCheck(McsCoupon couponEntity, long userId) {
...
//        }
        //用户是否超过限制
        int recordNum = this.count(new QueryWrapper<McsCouponRecord>()
