...
import com.doumies.mcs.service.ICouponRecordService;
...
public class CouponRecordController {
    ...
    public Result page(BasePageQuery pageQuery) {
        ...
        LambdaQueryWrapper<McsCouponRecord> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(McsCouponRecord::getUserId, userId).orderByDesc(McsCouponRecord::getGmtCreate);
        Page<McsCouponRecord> page = new Page<>(pageQuery.getPageNum(), pageQuery.getPageSize());
        IPage<McsCouponRecord> result = couponRecordService.page(page, queryWrapper);
    ...
    public Result detail(@ApiParam(value = "优惠券记录ID") @PathVariable("id") String id) {
        Long userId = JwtUtils.getUserId();
        QueryWrapper<McsCouponRecord> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("user_id", userId).eq("id", id);
        McsCouponRecord result = couponRecordService.getOne(queryWrapper);
        if (result == null) {
