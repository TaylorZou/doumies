...
public class CouponTemplateController {
    ...
    public Result page(Integer page, Integer limit, String name) {
        IPage<McsCouponTemplateVO> pageResult = couponTemplateService.pageQuery(page, limit, name);
    ...
    public Result<McsCouponTemplateInfoVO> info(@ApiParam(value = "优惠券模板ID", defaultValue = "1")
                                         @RequestParam("id") String id) {
        McsCouponTemplateInfoVO templateVO = couponTemplateService.info(id);
    ...
    public Result couponPage(@ApiParam(value = "优惠券领取使用条件分页查询") CouponPageQuery query) {
        IPage<McsCoupon> iPage = couponService.pageQuery(query);
