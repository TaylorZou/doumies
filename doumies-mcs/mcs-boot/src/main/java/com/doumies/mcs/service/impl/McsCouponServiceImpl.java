...
import com.doumies.mcs.mapper.McsCouponMapper;
...
import com.doumies.mcs.pojo.vo.CouponClassify;
import com.doumies.mcs.pojo.vo.CouponTemplateVO;
import com.doumies.mcs.pojo.vo.McsCouponVO;
import com.doumies.mcs.service.ICouponRedisService;
...
@Service
@Slf4j
public class McsCouponServiceImpl extends ServiceImpl<McsCouponMapper, McsCoupon> implements IMcsCouponService {
    ...
    public List<CouponTemplateVO> findAvailableTemplate(Long userId) {
        ...
        // 1、正在发放中的优惠券模板
        List<McsCouponTemplate> templates = couponTemplateService.findAllOfferingTemplate(userTypes);
        ...
        List<McsCoupon> coupons = findCouponsByState(userId, 0);
        Map<Long, List<McsCoupon>> couponMap = coupons.stream().collect(Collectors.groupingBy(McsCoupon::getTemplateId));
        log.info("All User Receive Coupon Template Size={}", couponMap.size());
        templates = templates.stream().filter(template -> {
            List<McsCoupon> receivedCoupons = couponMap.getOrDefault(template.getId(), new ArrayList<>());
    ...
    public void receive(Long userId, String templateId) {
        // 1、校验优惠券发放状态
        McsCouponTemplate template = couponTemplateService.findByTemplateId(templateId);
        ...
        // 2、校验用户领取状态
        List<McsCoupon> coupons = this.findByTemplateId(userId, templateId);
    ...
    @Override
    public List<McsCoupon> findCouponsByState(Long userId, Integer state) {
        log.info("根据查询优惠券列表，UserId={}, State={}", userId, state);
        List<McsCoupon> cachedCoupons = couponRedisService.getCachedCoupons(userId, state);
        List<McsCoupon> targetCoupon = new ArrayList<>();
        if (!CollUtil.isEmpty(cachedCoupons)) {
            // 如果缓存中有数据直接取缓存中的数据
            targetCoupon = cachedCoupons;
        } else {
            // 如果缓存中没有数据，从数据库取数据
            QueryWrapper<McsCoupon> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("user_id", userId);
            queryWrapper.eq("state", state);
            List<McsCoupon> dbCoupon = this.list(queryWrapper);
            if (CollUtil.isEmpty(dbCoupon)) {
                log.info("Current User Do Not Has Coupon: {} {}", userId, state);
                return dbCoupon;
            }
            List<McsCouponTemplate> templates = couponTemplateService.listByIds(dbCoupon.stream().map(McsCoupon::getTemplateId).collect(Collectors.toList()));
            Map<Long, McsCouponTemplate> templateMap = templates.stream().collect(Collectors.toMap(McsCouponTemplate::getId, Function.identity()));
    ...
    @Override
    public McsCouponVO detail(String couponId) {
        log.info("根据优惠券ID获取优惠券详情，couponId={}", couponId);
        McsCoupon coupon = this.getById(couponId);
        return BeanMapperUtils.map(coupon, McsCouponVO.class);
    }

    @Override
    public void add(CouponForm form) {
        log.info("新增优惠券，form={}", form);
        McsCoupon coupon = BeanMapperUtils.map(form, McsCoupon.class);
        coupon.setId(null);
        this.save(coupon);
    }
    ...
    @Override
    public IPage<McsCoupon> pageQuery(CouponPageQuery query) {
        log.info("Page Query Coupons，TemplateID={}，query={}", query.getTemplateId(), query);
        Page<McsCoupon> page = new Page<>(query.getPageNum(), query.getPageSize());
        QueryWrapper<McsCoupon> queryWrapper = new QueryWrapper<>();
     ...
     * @param couponCode
     */
    private boolean saveCouponRecord(McsCouponTemplate template, String couponCode) {
        log.info("Create Coupon Records, TemplateId={}, CouponCode={}", template.getId(), couponCode);
        McsCoupon coupon = new McsCoupon();
     ...
     * @return
     */
    private List<McsCoupon> findByTemplateId(Long userId, String templateId) {
        QueryWrapper<McsCoupon> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("user_id", userId);
