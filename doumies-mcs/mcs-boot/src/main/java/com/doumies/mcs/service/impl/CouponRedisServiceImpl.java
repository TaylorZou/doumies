...
import static com.doumies.common.constant.RedisConstants.*;
...
public class CouponRedisServiceImpl implements ICouponRedisService {
    ...
    @Override
    public List<McsCoupon> getCachedCoupons(Long userId, Integer state) {
        ...
        return couponCaches.stream()
                .map(obj -> JSONUtil.toBean(obj, McsCoupon.class))
                .collect(Collectors.toList());
    }
    ...
    public void saveEmptyCouponListToCache(Long userId, List<Integer> states) {
        ...
        // 构造无效的coupon
        invalidCouponMap.put("-1", JSONUtil.toJsonStr(new McsCoupon()));
    ...
    @Override
    public Integer addCouponToCache(Long userId, List<McsCoupon> coupons, Integer state) {
    ...
    private Integer addCouponToCacheForUsable(Long userId, List<McsCoupon> coupons) {
    ...
    private Integer addCouponToCacheForUsed(Long userId, List<McsCoupon> coupons) {
    ...
    private int addCouponToCacheForExpired(Long userId, List<McsCoupon> coupons) {
        ...
        List<McsCoupon> curUsableCoupons = getCachedCoupons(userId, CouponStateEnum.USABLE.getCode());
        List<McsCoupon> curExpiredCoupons = getCachedCoupons(userId, CouponStateEnum.EXPIRED.getCode());
        ...
        List<Long> curUsableIds = curUsableCoupons.stream().map(McsCoupon::getId).collect(Collectors.toList());
        List<Long> paramIds = coupons.stream().map(McsCoupon::getId).collect(Collectors.toList());
