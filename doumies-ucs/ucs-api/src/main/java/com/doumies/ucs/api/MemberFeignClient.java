...
@FeignClient(name = "doumies-ucs", contextId = "member")
public interface MemberFeignClient {

    @PostMapping("/app-api/v1/members")
    Result<Long> add(@RequestBody UcsMember member);

    @PutMapping("/app-api/v1/members/{id}")
    <T> Result<T> update(@PathVariable Long id, @RequestBody UcsMember member);
    ...
    Result<UcsMember> getUserEntityById(@PathVariable Long id);
