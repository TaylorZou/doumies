...
import com.doumies.common.enums.BusinessTypeEnum;
import com.doumies.common.redis.BusinessNoGenerator;
...
import com.doumies.ocs.config.WxPayProperties;
...
import com.doumies.ocs.service.ICartService;
...
import com.doumies.ucs.api.MemberAddressFeignClient;
...
import static com.doumies.ocs.constant.OcsConstants.*;

@RequiredArgsConstructor
@Slf4j
@Service
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OcsOrder> implements IOrderService {
    ...
    public OrderConfirmVO confirm(OrderConfirmDTO orderConfirmDTO) {
        ...
        CompletableFuture<Void> addressesCompletableFuture = CompletableFuture.runAsync(() -> {
            List<UcsAddress> addresses = addressFeignService.list(memberId).getData();
...
//        }
        ...
        // TCC模式创建订单(状态：待支付)
        OcsOrder order = seataTccOrderService.prepareSubmitOrder(null, submitDTO);
        // 将订单放入延时队列，超时未支付由交换机order.exchange切换到死信队列完成系统自动关单
        log.info("订单超时取消RabbitMQ消息发送，订单SN：{}", orderToken);
        rabbitTemplate.convertAndSend("order.exchange", "order.create", orderToken);
    ...
    public <T> T pay(Long orderId, String appId, PayTypeEnum payTypeEnum) {
        OcsOrder order = this.getById(orderId);
        if (order == null) {
    ...
    private Boolean balancePay(OcsOrder order) {
        // 扣减余额
    ...
    private WxPayUnifiedOrderV3Result.JsapiResult wxJsapiPay(String appId, OcsOrder order) {
        Long userId = JwtUtils.getUserId();
        Result<UcsMember> userInfoResult = memberFeignClient.getUserEntityById(userId);
        if (!Result.isSuccess(userInfoResult)) {
            throw new BizException("用户查询失败");
        }
        UcsMember userInfo = userInfoResult.getData();
        Long payAmount = order.getPayAmount();
    ...
    public boolean closeOrder(String orderToken) {
        ...
        OcsOrder order = this.getOne(new LambdaQueryWrapper<OcsOrder>()
                .eq(OcsOrder::getOrderSn, orderToken));
    ...
    public boolean cancelOrder(Long id) {
        ...
        OcsOrder order = this.getById(id);
        if (order == null) {
            throw new BizException("订单不存在");
        }
    ...
    public void handleWxPayOrderNotify(SignatureHeader signatureHeader, String notifyData) throws WxPayException {
        ...
        // 根据商户订单号查询订单
        QueryWrapper<OcsOrder> wrapper = new QueryWrapper<>();
        wrapper.lambda().eq(OcsOrder::getOutTradeNo, result.getOutTradeNo());
        OcsOrder orderDO = this.getOne(wrapper);
        // 支付成功处理
     ...
     */
    @Override
    public IPage<OcsOrder> listOrdersWithPage(OrderPageQuery queryParams) {
        Page<OcsOrder> page = new Page<>(queryParams.getPageNum(), queryParams.getPageSize());
        List<OcsOrder> list = this.baseMapper.listUsersWithPage(page, queryParams);
