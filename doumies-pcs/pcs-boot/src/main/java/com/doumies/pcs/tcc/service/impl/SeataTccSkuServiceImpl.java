package com.doumies.pcs.tcc.service.impl;
...
import com.doumies.pcs.tcc.idempotent.IdempotentUtil;
import com.doumies.pcs.tcc.service.SeataTccSkuService;
...
public class SeataTccSkuServiceImpl implements SeataTccSkuService {
    ...
    public boolean prepareSkuLockList(BusinessActionContext businessActionContext, List<LockStockDTO> skuLockList) {
        ...
        skuLockList.forEach(item -> {
            ...
            lock.lock();
            boolean result = iPcsSkuService.update(new LambdaUpdateWrapper<PcsSku>()
        ...
        if (CollectionUtil.isNotEmpty(unlockSkuList)) {
            ...
            lockSkuList.forEach(item ->
                    iPcsSkuService.update(new LambdaUpdateWrapper<PcsSku>()
                            .eq(PcsSku::getId, item.getSkuId())
                            .setSql("locked_stock = locked_stock - " + item.getCount()))
            );
            // 提示订单哪些商品库存不足
