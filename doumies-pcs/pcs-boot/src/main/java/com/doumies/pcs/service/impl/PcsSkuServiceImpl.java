...
import com.doumies.pcs.mapper.PcsSkuMapper;
...
public class PcsSkuServiceImpl extends ServiceImpl<PcsSkuMapper, PcsSku> implements IPcsSkuService {
    ...
    public Result lockStock(List<LockStockDTO> skuLockList) {
        ...
        skuLockList.forEach(item -> {
            RLock lock = redissonClient.getLock(PcsConstants.LOCK_SKU_PREFIX + item.getSkuId()); // 获取商品的分布式锁
            lock.lock();
            boolean result = this.update(new LambdaUpdateWrapper<PcsSku>()
                    .setSql("locked_stock = locked_stock + " + item.getCount())
                    .eq(PcsSku::getId, item.getSkuId())
        ...
        if (CollectionUtil.isNotEmpty(unlockSkuList)) {
            ...
            lockSkuList.forEach(item ->
                    this.update(new LambdaUpdateWrapper<PcsSku>()
                            .eq(PcsSku::getId, item.getSkuId())
                            .setSql("locked_stock = locked_stock - " + item.getCount()))
            );
            // 提示订单哪些商品库存不足
        ...
        redisTemplate.opsForValue().set(PcsConstants.LOCKED_STOCK_PREFIX + orderToken, JSONUtil.toJsonStr(skuLockList));
    ...
    public boolean unlockStock(String orderToken) {
        ...
        String json = redisTemplate.opsForValue().get(PcsConstants.LOCKED_STOCK_PREFIX + orderToken);
        ...
        // 删除redis中锁定的库存
        redisTemplate.delete(PcsConstants.LOCKED_STOCK_PREFIX + orderToken);
        return true;
    }
    ...
    public Integer getStockById(Long id) {
        Integer stock = 0;
        // 读->缓存
        Object cacheVal = redisTemplate.opsForValue().get(PcsConstants.LOCKED_STOCK_PREFIX + id);
        if (cacheVal != null) {
        ...
        // 读->数据库
        PcsSku pcsSku = this.getOne(new LambdaQueryWrapper<PcsSku>()
                .eq(PcsSku::getId, id)
                .select(PcsSku::getStock));

        if (pcsSku != null) {
            stock = pcsSku.getStock();
            // 写->缓存
            redisTemplate.opsForValue().set(PcsConstants.LOCKED_STOCK_PREFIX + id, String.valueOf(stock));
        }
